...
class WaveGreeterImshow:
    def __init__(self):
        ...
        self.confirmed_pub = rospy.Publisher("/guest_confirmed_count", Int32, queue_size=10) #latch=True)
        ...
        self.debug_pub = rospy.Publisher("/guest_counter/debug_image", Image, queue_size=1)
                # --- TTS (TIAGo) ---
        self.enable_tts = bool(rospy.get_param("~enable_tts", False))   # 是否启用说话
    ...
    def _annotate(self, det_r0, visible_count, pose_r0=None, debug_wave=None):
        ...
        if self.state == "CONFIRMED":
            cv2.putText(img, f"CONFIRMED={self.confirmed_count}", (20, 105),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2)
        else:
            # 挥手调试信息，调参
    ...
    def cb(self, msg):
        ...
        if self.state == "WAIT_WAVE":
            ...
            if wave_now and visible_count > 0:
                ...
                rospy.loginfo("Wave trigger -> CONFIRMED, confirmed_count=%d", self.confirmed_count)
